...
Section Rename.
  ...
  Fixpoint renameModules (m: Modules) :=
    match m with
      | RegFile dataArray read write IdxBits Data init => RegFile (rename dataArray) (map rename read) (rename write) init
  ...
  Proof.
    ...
    - reflexivity.
    - specialize (IHHMultistepBeh rename1To1 m' eq_refl eq_refl).
      dest; subst.
      apply renameStepRev in HStep.
      dest; subst.
      rewrite <- renameMapUnion.
